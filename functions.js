...
function channelHasVideo(videoTitle, channel) {
  // Your code here
  return channel.videos.some((vidTitle) => vidTitle.title === videoTitle);
}
...
function getChannelByName(channelName, channels) {
  // Your code here
  return channels.find((chName) => chName.name === channelName);
}
...
function getChannelByVideoTitle(videoTitle, channels) {
  ...
  return channels.find((
    channel //can be replaced by .filter
  ) => channel.videos.some((vidTitle) => vidTitle.title === videoTitle));
}
...
function searchChannels(query, channels) {
  // Your code here
  return channels.filter((chName) => {
    if (chName.description.includes(query)) {
      return chName;
    }
    if (chName.name.includes(query)) {
      return chName;
    }
  });
}
